...
<body class="bg-gray-100">
    ...
    <div id="authModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form action="register.php" id="registerForm" class="hidden" method="post">
                ...
                <input name="apellido_P" id="apellido_P" type="text" placeholder="Apellido paterno"
                    class="w-full p-2 mb-2 border rounded" required>
                <input name="apellido_M" id="apellido_M" type="text" placeholder="Apellido materno"
                    class="w-full p-2 mb-2 border rounded" required>
                <input name="nombre_usuario" id="nombre_usuario" type="text" placeholder="Nombre de usuario"
                    class="w-full p-2 mb-2 border rounded" required>
                <input name="email" type="email" id="email" placeholder="Correo" class="w-full p-2 mb-2 border rounded"
                    required>
                <input name="contrasena" type="password" id="contrasena" placeholder="Contraseña"
                    class="w-full p-2 mb-2 border rounded" required>
                <input name="confirmar_contrasena" id="contrasena2" type="password" placeholder="Confirmar Contraseña"
                    class="w-full p-2 mb-2 border rounded" required>
                <div class="mb-2">
                    <label>Género:</label>
                ...
                <input name="fecha_Nacimiento" id="fecha_Nacimiento" type="date" class="w-full p-2 mb-2 border rounded"
    ...
    <script>
        ...
        document.getElementById('registerForm').addEventListener('submit', function (e) {
            ...
            //validar que el nombre no tenga caracteres especiales ni numeros
            if (/[^a-zA-Z\s]/.test(nombre) || /[0-9]/.test(nombre)) {
                alert('El nombre no debe contener números y/o caracteres especiales.');
                return;
            }
            //validar que el apellido no tenga caracteres especiales ni numeros
            if (/[^a-zA-Z\s]/.test(apellidoP) || /[^a-zA-Z\s]/.test(apellidoM) || /[0-9]/.test(apellidoP) || /[0-9]/.test(apellidoM)) {
                alert('El apellido no debe contener números y/o caracteres especiales.');
                return;
            }
